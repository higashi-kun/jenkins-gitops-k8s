...
プランに合意いただけたら **Phase 0(Go アプリ + Dockerfile + リポジトリ雛形)** から着手します。

決定事項：
- ✅ コンテナツール：Podman(rootless・daemonless)
- ✅ ArgoCD 導入：Phase 5(kubectl 直デプロイ)→ Phase 6(GitOps 移行)の段階踏み方式
- ✅ 各フェーズは「その時点で動作すること」を完了条件(DoD)で保証する拡張型構成
- ✅ Jenkins 配置：Phase 2〜5 は **VM ベース**(Controller + Agent / Azure Linux VM)。いきなり AKS 上 Jenkins はハードルが高いため、Phase 7 拡張ネタとして留める

運用ルール：
- **本ドキュメントの作成者(AI)は、ユーザから「実装して」と明示的に指示されるまでコード・IaC・マニフェスト等の実装を一切行わない**
- 各 Phase の着手タイミング・範囲はユーザが決定し、指示した場合のみ実装に移る
- 実装指示がない間は、プランの調整・コーチング・質問への応答のみを行う
